...
class $modify(GameLevelManagerExt, GameLevelManager) {
	static void updateLevelByJson(GJGameLevel* level) {
		...
		value.try_set("m_levelName", level->m_levelName);
		...
		value.try_set("m_levelDesc", level->m_levelDesc);
		value.try_set("m_creatorName", level->m_creatorName);
		...
		value.try_set("m_capacityString", level->m_capacityString);
		value.try_set("m_songIDs", level->m_songIDs);
		value.try_set("m_sfxIDs", level->m_sfxIDs);
		//save json
